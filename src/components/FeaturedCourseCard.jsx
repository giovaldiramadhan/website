...
const FeaturedCourseCard = ({ course }) => {
    ...
    const fetchImage = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/image-proxy?query=${category}`
        );
  ...
  const cardStyle = useMemo(() => {
    return imageUrl && imageUrl !== 'gradient'
      ? { backgroundImage: `url(${imageUrl})` }
      : {};
  }, [imageUrl]);

  return (
    <CardLink to={`/courses/${id}`} aria-label={`Go to course ${course_name}`}>
      <CardWrapper style={cardStyle}>
        {(loading || !imageUrl || imageUrl === 'gradient') && (
